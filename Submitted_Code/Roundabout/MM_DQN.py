...
class LidarToOccupancyGrid:
    ...
    def process(self, obs, angles, v_vel, heading):
        ...
        occ = self.occupancy_grid[50:200,50:200]/ 16
...
class Network(nn.Module):
    def __init__(self, env1, env2, device, depths1, depths2, final_layer):
        super().__init__()
    ...
    def save(self, epoch):
        print('model saved')
        th.save(self.state_dict(), 'training_model/MM_DQN_R_' + str(epoch) + '.pth')

    def load(self, epoch):
        print('load model')
        self.load_state_dict(th.load('training_model/MM_DQN_R_' + str(epoch) + '.pth'))
...
online_net = Network(env,env2, device=device, depths1=(8, 16,16), depths2=(8, 16,16), final_layer=512)
target_net = Network(env,env2, device=device, depths1=(8, 16,16), depths2=(8, 16,16), final_layer=512)
