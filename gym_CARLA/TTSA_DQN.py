...
@dataclass
class Args:
    exp_name: str = "carla_multimodal_ttsa_dqn"
    seed: int = 0
...
class TTSA(nn.Module):
    def __init__(self, dim, num_heads, num_steps):
        ...
        self.q_lif1 = snntorch.Leaky(0.6, 1)
        self.q_lif2 = snntorch.Leaky(0.6, 4)

        self.k_conv = layer_init(nn.Linear(dim, dim))
        self.k_lif1 = snntorch.Leaky(0.6, 1)
        self.k_lif2 = snntorch.Leaky(0.6, 4)

        self.v_conv = layer_init(nn.Linear(dim, dim))

        self.blif = snntorch.Leaky(0.6, 1)
    ...
    def forward(self, q, k, v):
        ...
        for step in range(self.num_steps):
            b, n, c = q[step].shape
            cur_q = self.q_conv(q[step])
            q1, memq1 = self.q_lif1(cur_q, memq1)
            q2, memq2 = self.q_lif2(-cur_q, memq2)
            q_out = q1 - q2
            q_heads = q_out.reshape(b, n, self.num_heads, c // self.num_heads).permute(0, 2, 1, 3).contiguous()

            cur_k = self.k_conv(k[step])
            k1, memk1 = self.k_lif1(cur_k, memk1)
            k2, memk2 = self.k_lif2(-cur_k, memk2)
            k_out = k1 - k2
            k_heads = k_out.reshape(b, n, self.num_heads, c // self.num_heads).permute(0, 2, 1, 3).contiguous()

            cur_v = self.v_conv(v[step])
            v_heads = cur_v.reshape(b, n, self.num_heads, c // self.num_heads).permute(0, 2, 1, 3).contiguous()

            attn = (q_heads @ k_heads.transpose(-2, -1)) * self.scale
            spk_attn, mem_bin = self.blif(attn, mem_bin)
            x = spk_attn @ v_heads
...
if __name__ == "__main__":
    ...
    for global_step in trange(args.total_timesteps, desc="TTSA-DQN Training"):
